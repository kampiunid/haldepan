...
const getData = async (page, cat, type) => {
  const baseURL = process.env.NEXTAUTH_URL;

  const res = await fetch(
    `${baseURL}/api/posts?page=${page}&cat=${cat || ""}&type=${type || ""}`,
    {
      cache: "no-store",
    }
  );
...
const CardList = async ({ page, cat, dark, type, tittle }) => {
  ...
  const { posts, count } = await getData(page, cat, type);
